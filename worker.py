...
@gin.configurable('simple_worker', whitelist=['trainer_class', 'eval_score'])
class SimpleWorker(mp.Process):
    def __init__(self, is_stop_requested, train_queue, score_queue, finished_queue,
                 gpu_id, worker_id, gin_string, trainer_class=None, eval_score=gin.REQUIRED):
        super().__init__()
        print("Init Worker")
    ...
    def train_task(self, task):
        ...
        try:
            ...
            self.set_rng_states(task["random_states"])
            dataset = np.load(task['dataset_path'], allow_pickle=True)
    ...
    def eval_task(self, task):
        ...
        try:
            ...
            # Train
            trainer.set_id(task['id'])
            score = trainer.eval(dataset=dataset)
            task['score'] = score
            task['random_states'] = self.get_rng_states()
